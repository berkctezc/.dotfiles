...
alias localip="if ip addr show en0 >/dev/null 2>&1; then ip addr show en0 | grep 'inet ' | awk '{print \$2}' | cut -d/ -f1; else hostname -I | awk '{print \$1}'; fi"alias ips "ifconfig -a | grep -o 'inet6\? \(addr:\)\?\s\?\(\(\([0-9]\+\.\)\{3\}[0-9]\+\)\|[a-fA-F0-9:]\+\)' | awk '{ sub(/inet6? (addr:)? ?/, \"\"); print }'"
